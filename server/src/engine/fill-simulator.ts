...
export class FillSimulator extends Effect.Service<FillSimulator>()("FillSimulator", {
  succeed: {
    simulate(
      _side: "BUY" | "SELL",
      _tokenId: string,
      ...
      if (levels.length === 0) {
        return { filled: false, filledShares: 0, avgPrice: 0, fee: 0, levels: [], reason: "no_liquidity" };
      }

      const totalAvailable = levels
        .filter((l) => _side === "BUY" ? l.price <= limitPrice : l.price >= limitPrice)
        .reduce((s, l) => s + l.size, 0);

      if (totalAvailable < requestedShares * o.minLiquidityPct) {
        return { filled: false, filledShares: 0, avgPrice: 0, fee: 0, levels: [], reason: "insufficient_liquidity" };
      }

      if (Math.random() > o.fillProbability) {
        return { filled: false, filledShares: 0, avgPrice: 0, fee: 0, levels: [], reason: "queue_position_miss" };
      }
      ...
      for (const level of levels) {
        ...
        const priceOk = _side === "BUY" ? level.price <= limitPrice : level.price >= limitPrice;
        if (!priceOk) break;
        ...
        totalFee += calculateFeeStatic(fillAtLevel, effectivePrice);
      ...
      if (filledShares <= 0) {
        return { filled: false, filledShares: 0, avgPrice: 0, fee: 0, levels: [], reason: "price_exceeded" };
      }
      ...
      return {
        ...
        fee: totalFee,
        levels: filledLevels,
        reason: filledShares < requestedShares ? "partial_fill" : undefined,
      };
    },
  },
}) {}
